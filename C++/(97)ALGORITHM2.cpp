...
int main (){
	...
	//equal = Verifica se os elementos de uma coleção são iguais aos de outra(posições e valores)
	cout << "EQUAL" << endl;
	...
	//copy = Copia elementos de uma coleção em outra, mas nçai cria novas posições
	cout << "COPY" << endl;
	...
	cout << "COPY_IF" << endl;
	copy_if(vt5.begin(),vt5.end(),vt1.begin(),[](int i){return (i%2==0);}); //Copia so os pares
	for(auto x:vt1){
        cout << x << "  ";
	}
    ...
    //move = Move elementos de uma coleçãopara outra
    cout << "MOVE" << endl;
    vt4=move(vt1);
    for(auto x:vt4){
        cout << x << "  ";
