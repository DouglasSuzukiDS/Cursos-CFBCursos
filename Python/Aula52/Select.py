...
########## Criar Conexão
def ConexaoBanco():
    caminho = "C:\\Users\\Krzysztof\\Documents\\Python\\Aula52\\banco\\agenda.db"
    con = None
    try:
...
def consulta(conexao, sql):
    c = conexao.cursor()
    c.execute(sql)
    resultado = c.fetchall() #Retorna os registros encontrados
    return resultado

#vsql = "SELECT * FROM tb_contatos" #Todos os registros
#vsql = "SELECT * FROM tb_contatos WHERE N_IDCONTATO = 2" #Campo selecionado
vsql = "SELECT * FROM tb_contatos WHERE T_NOMECONTATO LIKE 'Br%' " 
