...
def popular():
    tv.delete(*tv.get_children()) #Pega todos os filhos
...
def pesquisar():
    ...
    vquery = "SELECT * FROM tb_nomes WHERE nome LIKE '%" + vnomepesquisar.get() + "%' order by ID" #Contendo o digitado
    linhas = Banco.dql(vquery)
    for i in linhas:
        tv.insert("", "end", values = i)
...
tv = ttk.Treeview(quadroGrid, column = ('id', 'nome', 'fone'), show = 'headings')
